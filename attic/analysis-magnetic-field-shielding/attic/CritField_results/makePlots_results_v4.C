...
makePlots_results_v4()
{
  ...
  // -----
  // 95% Shielding Fractions

  TH1F* htemp_xl = new TH1F("htemp_xl","",501,-0.5,500.5);

  htemp_xl->Reset("ICES");                                                                                        
  t_5l->Draw("bext >> htemp_xl","shielding_factor>0.95");                                                       
  cout << "B95 for 5 layer short: " << htemp_xl->GetBinCenter(htemp_xl->FindLastBinAbove()) << endl; 

  htemp_xl->Reset("ICES");                                                                                        
  t_5l_long->Draw("bext >> htemp_xl","shielding_factor>0.95");  
  cout << "B95 for 5 layer long: " << htemp_xl->GetBinCenter(htemp_xl->FindLastBinAbove()) << endl; 
