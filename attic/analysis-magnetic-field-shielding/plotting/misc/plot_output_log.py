...
#    interval = np.linspace(i0lst[i],i1lst[i],len(B_ext[i0lst[i]:i1lst[i]]))
#    popt, pcov = curve_fit(expfunc, interval, B_ext[i0lst[i]:i1lst[i]])
#    xlog += [popt[0]]

#    plt.figure()
#    plt.plot(interval, B_ext[i0lst[i]:i1lst[i]], 'ko', label="Original Noised Data")
#    plt.plot(interval, expfunc(interval, *popt), 'r-', label="Fitted Curve")
#    plt.legend()
#    plt.show()
