...
# B vs z scans using the long superconductor shield inside liquid Nitrogen bath in beam pipe segment surrounded by a single steering magnet
...
data_BvsZ_warm_MagnetOff_1 = np.genfromtxt('DATA_Gaussmeter/DataFile_151211_215638.txt', delimiter='\t')

# B vs z room temperature magnets ON
data_BvsZ_warm_MagnetOn_1 = np.genfromtxt('DATA_Gaussmeter/DataFile_151211_212410.txt', delimiter='\t')

# B vs z room temperature magnets OFF
data_BvsZ_warm_MagnetOff_2 = np.genfromtxt('DATA_Gaussmeter/DataFile_151211_225107.txt', delimiter='\t')

# Cooled, magnets OFF
data_BvsZ_cryo_MagnetOff_1 = np.genfromtxt('DATA_Gaussmeter/DataFile_151212_000953.txt', delimiter='\t')

# Cooled, magnets OFF
data_BvsZ_cryo_MagnetOff_2 = np.genfromtxt('DATA_Gaussmeter/DataFile_151212_005623.txt', delimiter='\t')

# Cooled, magnets ON
data_BvsZ_cryo_MagnetOn_1 = np.genfromtxt('DATA_Gaussmeter/DataFile_151212_020245.txt', delimiter='\t')

# print sums
print -1*(data_BvsZ_warm_MagnetOff_1[0,0]-945)
print -1*(data_BvsZ_warm_MagnetOff_1[30,0]-945)
print -1*(data_BvsZ_warm_MagnetOff_1[120,0]-945)

print data_BvsZ_warm_MagnetOff_1 [0:120,1].sum()
print data_BvsZ_warm_MagnetOff_2[0:120,1].sum()
print data_BvsZ_warm_MagnetOn_1[0:120,1].sum()

print data_BvsZ_cryo_MagnetOff_1 [0:120,1].sum()
print data_BvsZ_cryo_MagnetOff_2[0:120,1].sum()
...
plt.plot( -1.0*(data_BvsZ_warm_MagnetOff_2[:,0]-cstat_center) , -1*data_BvsZ_warm_MagnetOff_2[:,1], 'yo' , label='MagnetOff 2 (warm)')

plt.plot( -1.0*(data_BvsZ_cryo_MagnetOn_1[:,0]-cstat_center) , -1*data_BvsZ_cryo_MagnetOn_1[:,1], 'ro' , label='MagnetON (cryo)')
plt.plot( -1.0*(data_BvsZ_cryo_MagnetOff_1[:,0]-cstat_center) , -1*data_BvsZ_cryo_MagnetOff_1[:,1], 'co' , label='MagnetOff (cryo)')
plt.plot( -1.0*(data_BvsZ_cryo_MagnetOff_2[:,0]-cstat_center) , -1*data_BvsZ_cryo_MagnetOff_2[:,1], 'ko' , label='MagnetOff 2 (cryo)')
...
#Differnece magnet ON - OFF
#diff_warm = data_BvsZ_warm_MagnetOn_1[:,1] - data_BvsZ_warm_MagnetOff_1[:,1]
...
for z in zpos_warm:
    newval = data_BvsZ_warm_MagnetOn_1[ data_BvsZ_warm_MagnetOn_1[:,0] == z,1] - data_BvsZ_warm_MagnetOff_2[ data_BvsZ_warm_MagnetOff_2[:,0] == z,1]
...
for z in zpos_cryo:
    newval = data_BvsZ_cryo_MagnetOn_1[ data_BvsZ_cryo_MagnetOn_1[:,0] == z,1] - data_BvsZ_cryo_MagnetOff_1[ data_BvsZ_cryo_MagnetOff_1[:,0] == z,1]
...
plt.savefig('plots/BvZ_1.png', bbox_inches='tight')
plt.savefig('plots/BvZ_1.eps', bbox_inches='tight')
plt.show()
